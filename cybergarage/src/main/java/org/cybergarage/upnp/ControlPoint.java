...
public class ControlPoint implements HTTPRequestListener {
    ...
    public void removeExpiredDevices() {
        ...
        for (int n = 0; n < devCnt; n++) {
            if (dev[n].isExpired()) {
                KLog.d("Expired device = " + dev[n].getFriendlyName());
                removeDevice(dev[n]);
            }
        }
    }
    ...
    public void performNotifyListener(SSDPPacket ssdpPacket) {
        ...
        for (int n = 0; n < listenerSize; n++) {
            ...
            try {
                ...
                KLog.w("NotifyListener returned an error:", e);
            }
        }
    }
    ...
    public void performSearchResponseListener(SSDPPacket ssdpPacket) {
        ...
        for (int n = 0; n < listenerSize; n++) {
            ...
            try {
                ...
                KLog.w("SearchResponseListener returned an error:", e);
            }
...
//        httpServerList.start();
        ...
        // Notify Socket
        // 常驻服务
        // 接收组播消息，接收其他设备主动发出的notify/byebye消息
        ////////////////////////////////////////
    ...
    public void print() {
        DeviceList devList = getDeviceList();
        int devCnt = devList.size();
        KLog.d("Device Num = " + devCnt);
        for (int n = 0; n < devCnt; n++) {
            Device dev = devList.getDevice(n);
            KLog.d("[" + n + "] " + dev.getFriendlyName() + ", " + dev.getLeaseTime() + ", " + dev.getElapsedTime());
        }
    }
}
